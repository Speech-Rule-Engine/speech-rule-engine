...
import { SemanticProofParser } from './semantic_proof.js';
...
export class SemanticProcessor {
  ...
  public static proof(
    ...
    return new SemanticProofParser(
      SemanticProcessor.getInstance().factory_,
      parse,
      (nodes: SemanticNode[]) => SemanticProcessor.getInstance().row(nodes)
    ).proof(node, semantics);
  }
  ...
  private wrapPostfix(split: SemanticUtil.Slice) {
    ...
    if (split.div?.role === SemanticRole.POSTFIXOP) {
      if (!split.tail.length || split.tail[0].type === SemanticType.OPERATOR) {
        split.head = [
          SemanticProcessor.getInstance().postfixNode_(
  ...
  private getPunctuationInRow_(nodes: SemanticNode[]): SemanticNode[] {
    ...
    return newNodes.length === 1 && partition.rel.length === 1
      ? newNodes
      : [
          SemanticProcessor.getInstance().punctuatedNode_(
            newNodes,
            partition.rel
          )
        ];
