...
/**
 * Optional, additional condition excluding nodes that were generated from a
 * MathML mrow explicitly marked with `data-mjx-texclass="ORD"`. Such an mrow
 * (e.g., as generated by MathJax) signals that its content is intentionally
 * kept as a single "ordinary" atom and should not be reinterpreted by this
 * heuristic. For example, in `<mi>a</mi><mrow data-mjx-texclass="ORD">
 * <mo>+</mo><mi>y</mi></mrow>` the `+` stays a prefix operator instead of
 * being rewritten into an infix operation with `a`.
 *
 * @param node The node to test.
 * @returns True if the node is not marked as an ordinal group.
 */
function isNotOrdGroup(node: SemanticNode): boolean {
  return !SemanticHeuristics.flags.ord
    || node.attributes['texclass'] !== 'ORD';
}
...
function isRealPrefix(node: SemanticNode): boolean {
  return (
    SemanticPred.isType(node, SemanticType.PREFIXOP) &&
    isVisibleOperator(node) &&
    isNotOrdGroup(node)
  );
}
