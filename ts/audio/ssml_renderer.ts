...
export class SsmlRenderer extends XmlRenderer {
  ...
  public finalize(str: string) {
    return (
      ...
      ` xml:lang="${Engine.getInstance().features.get(EngineConst.Features.LOCALE)}">` +
      '<prosody rate="' +
  ...
  public merge(spans: Span[]): string {
    ...
    for (let i = 0; i < spans.length; i++) {
      ...
      const id = Engine.getInstance().features.is(EngineConst.Features.AUTOMARK)
        ? span.attributes['id']
        : Engine.getInstance().features.is(EngineConst.Features.MARK)
      ...
      if (
        Engine.getInstance().features.is(EngineConst.Features.CHARACTER) &&
