 ...
 * new empty elements.
 * @param {!Array.<!sre.SemanticNode>} nodes The operands.
 * @param {!Array.<sre.SemanticNode>} ops A list of invisible times operators.
 * @private
 */
sre.SemanticProcessor.matchSpaces_ = function(nodes, ops) {
  for (var i = 0, op; op = ops[i]; i++) {
    ...
    var sibling = /** @type {Element} */(mt1.nextSibling);
 ...
 * @param {Element} node The mml element.
 * @return {Element} The space element if it exists.
 * @private
 */
sre.SemanticProcessor.getSpacer_ = function(node) {
  ...
  while (sre.SemanticUtil.hasEmptyTag(node) &&
    ...
    node = /** @type {Element} */(node.childNodes[0]);
    if (sre.DomUtil.tagName(node) === 'MSPACE') {
      return node;
    }
  }
...
sre.SemanticProcessor.prototype.getFencesInRow_ = function(nodes) {
  ...
  return sre.SemanticProcessor.getInstance().fencesNew_(
      partition.rel, partition.comp, [], [felem]);
};
...
// TODO: (simons) Integrate this better.
/**
 * Recursively processes a list of nodes and combines all the fenced expressions
 * into single nodes. It also processes singular fences, building expressions
 * that are only fenced left or right.
 ...
 * @private
 */
sre.SemanticProcessor.prototype.fencesNew_ = function(
    fences, content, openStack, contentStack) {
  // Base case 1: Everything is used up.
  if (fences.length == 0 && openStack.length == 0) {
    return contentStack[0];
  ...
  if (fences.length == 0) {
    ...
    // 3. Repeat until fence stack is exhausted.
    // Push rightmost elements onto the result.
    return this.openNeutralFences_(openStack, contentStack);
  }
  var lastOpen = openStack[openStack.length - 1];
  var firstRole = fences[0].role;
  // General opening case.
  ...
  if (firstRole == sre.SemanticAttr.Role.OPEN ||
    ...
    openStack.push(fences.shift());
    contentStack.push(content.shift() || []);  // This is for type only!
    return this.fencesNew_(fences, content, openStack, contentStack);
  }
  // General closing case.
  if (lastOpen && (
      // Closing fence for some opening fence.
  ...
  if (lastOpen && firstRole == sre.SemanticAttr.Role.CLOSE &&
    ...
    var rightContent = contentStack.pop();
    var cutLength = contentStack.length - split.tail.length + 1;
    var innerNodes = this.neutralFencesNew_(
        split.tail, contentStack.slice(cutLength));
    contentStack = contentStack.slice(0, cutLength);
    var fenced = this.horizontalFencedNode_(
        split.div, fences.shift(),
        contentStack.pop().concat(innerNodes, rightContent));
    contentStack.push(contentStack.pop().concat([fenced], content.shift()));
    return this.fencesNew_(fences, content, split.head, contentStack);
  }
  sre.SemanticProcessor.printNodeList__('Closing fences', fences);
  // Final Case: A singular closing fence.
  // We turn the fence into a punctuation.
  var fenced = fences.shift();
  sre.SemanticProcessor.fenceToPunct_(fenced);
...
sre.SemanticProcessor.prototype.openNeutralFences_ = function(fences, content) {
  var openPred = sre.SemanticPred.isAttribute('role', 'OPEN');
  var result = content.shift();
  // var partition = sre.SemanticProcessor.partitionNodes_(openStack, openPred);
  ...
  while (fences.length > 0) {
    var split = sre.SemanticProcessor.sliceNodes_(fences, openPred);
    ...
    var cutLength = split.head.length - 1;
    var neutral = this.neutralFencesNew_(
 ...
 * @param {!Array.<!Array.<sre.SemanticNode>>} content Intermediate
 *     content. Observe that |content| = |fences| - 1
 * @return {{fences: !Array.<sre.SemanticNode>, content:
 *           !Array.<!Array.<sre.SemanticNode>>}} The list of remaining
 *           fences and content.
 * @private
 */
sre.SemanticProcessor.prototype.neutralFencesNew_ = function(fences, content) {
...
//
sre.SemanticProcessor.prototype.combineFencesEagerly_ = function(fences, content) {
  ...
  if (fences.length === 1) {
    ...
    sre.SemanticProcessor.printNodeList__('Content now', content);
    return newContent;
  }
  var fenced = this.horizontalFencedNode_(
 ...
 * @private
 */
sre.SemanticProcessor.prototype.combineFencedContentNew_ = function(
    leftFence, rightFence, midFences, content) {
  ...
  content = content.slice(cutLength);
  var rightContent = content.shift();
  //var innerNodes = this.neutralFencesNew_(midFences, midContent);
  var neutral = this.neutralFencesNew_(midFences, midContent);
...
sre.SemanticProcessor.prototype.horizontalFencedNode_ = function(
  ...
  if (ofence.role === sre.SemanticAttr.Role.OPEN) {
    // TODO: (Simons) Do we need this for bra-ket?
...
sre.SemanticProcessor.MML_TO_BOUNDS_ = {
  ...
  'MUNDEROVER': {
    type: sre.SemanticAttr.Type.UNDERSCORE, length: 2, accent: true}
};
 ...
 * @param {!Array.<sre.SemanticNode>} children All children, where center is
 *     first node.
 * @param {sre.SemanticNode|undefined} innerNode The innermost node if it
 *     exists.
 * @param {sre.SemanticAttr.Type} type The new node type.
 * @return {!sre.SemanticNode} The newly created limit node.
 * @private
 ...
 */
sre.SemanticProcessor.printNodeList__ = function(title, nodes) {
  console.log(title);
