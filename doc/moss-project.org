...
** Preferences are effectively a set of dynamic constraints on their own.

*** Each preference is an axis

*** Each axis has a number of values to choose from

*** Equate Auto to default in the comparator.

** MathJax integration: Use submenus and radio buttons.

** Tests and things that can only be handled after the Simons project:
   They have mainly to do with necessary improvements and restructuring of
   semantic heuristics.
*** Trig030, Trig036: Needs better handling of degrees as postfix function.
*** Exp034: Needs a heuristic to pull the negation sign into the (mixed) number.
