...
\texttt{comma, ellipsis, fullstop, dash, prime, unknown}

\texttt{vbar, openfence, closefence} are punctuation elements, where we have a
fence (see next paragraph), which is not used as a fence, i.e., it is solitary
and has no counterpart.

\texttt{dummy} is the usage of invisible comma as the dummy separator for text.

\texttt{application} is the usage of unicode function application symbol.
...
\paragraph{\texttt{number}}

\texttt{integer} exclusively numerical.

\texttt{float} numerical with punctuation.

\texttt{othernumber} other numbers, that might contain alpha characters, etc.

\texttt{mixed} An integer with an vulgar fraction and an implicit multiplication between the two.

\paragraph{\texttt{unit}}

When a term has been identified as type \texttt{unit} and is also assigned the
